...
for t = tau:tau:t_e
    f = zeros(1, x_steps-2)';
    for i = 2:x_steps-3
        f(i) = (h^2-2*(1-xhi)*tau*a)*u_c(i)+(1-xhi)*tau*a*u_c(i+1)+(1-xhi)*tau*a*u_c(i-1)+tau*h^2*sin(t);
    end
    u_0 = exact(t-tau, x_0);
    u_e = exact(t-tau, x_e);
    u_c0 = exact(t, x_0);
    u_ce = exact(t, x_e);
    f(1) = (h^2-2*(1-xhi)*tau*a)*u_c(1)+(1-xhi)*tau*a*u_c(2)+(1-xhi)*tau*a*u_0+xhi*tau*a*u_c0+tau*h^2*sin(t);
    f(x_steps-2) = (h^2-2*(1-xhi)*tau*a)*u_c(x_steps-2)+(1-xhi)*tau*a*u_e+(1-xhi)*tau*a*u_c(x_steps-3)+xhi*tau*a*u_ce+tau*h^2*sin(t);
    u_c = linsolve(A, f)';
